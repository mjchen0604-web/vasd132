...
func (AntigravityAuthenticator) Login(ctx context.Context, cfg *config.Config, opts *LoginOptions) (*coreauth.Auth, error) {
	...
	clientID, clientSecret, errResolve := resolveAntigravityOAuthClient(cfg)
	if errResolve != nil {
...
waitForCallback:
	...
	tokenResp, errToken := exchangeAntigravityCode(ctx, cbRes.Code, redirectURI, httpClient, clientID, clientSecret)
	if errToken != nil {
...
func exchangeAntigravityCode(ctx context.Context, code, redirectURI string, httpClient *http.Client, clientID, clientSecret string) (*antigravityTokenResponse, error) {
	data := url.Values{}
	data.Set("code", code)
