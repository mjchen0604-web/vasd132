...
func resolveGeminiOAuthClient(cfg *config.Config) (string, string, error) {
	if cfg == nil {
		return "", "", fmt.Errorf("gemini oauth: configuration is required")
	}
	clientID := strings.TrimSpace(cfg.OAuth.Gemini.ClientID)
	clientSecret := strings.TrimSpace(cfg.OAuth.Gemini.ClientSecret)
	if clientID == "" || clientSecret == "" {
		return "", "", fmt.Errorf("gemini oauth: missing oauth.gemini.client-id or oauth.gemini.client-secret")
	}
	return clientID, clientSecret, nil
}

func resolveAntigravityOAuthClient(cfg *config.Config) (string, string, error) {
	if cfg == nil {
		return "", "", fmt.Errorf("antigravity oauth: configuration is required")
	}
	clientID := strings.TrimSpace(cfg.OAuth.Antigravity.ClientID)
	clientSecret := strings.TrimSpace(cfg.OAuth.Antigravity.ClientSecret)
	if clientID == "" || clientSecret == "" {
		return "", "", fmt.Errorf("antigravity oauth: missing oauth.antigravity.client-id or oauth.antigravity.client-secret")
	}
	return clientID, clientSecret, nil
}
...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	...
	clientID, clientSecret, errResolve := resolveGeminiOAuthClient(h.cfg)
	if errResolve != nil {
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	const antigravityCallbackPort = 51121
	...
	clientID, clientSecret, errResolve := resolveAntigravityOAuthClient(h.cfg)
	if errResolve != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": errResolve.Error()})
		return
	}
